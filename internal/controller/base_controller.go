...
func (self *BaseController[T]) Reconcile(ctx context.Context, req ctrl.Request, obj T) (ctrl.Result, error) {
	...
	if IsUpsert(obj) {
		...
		if self.StatusID != nil && self.StatusID(obj) == "" {
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Creating", "Create", "Creating %s", objName)
			if err := self.Create(ctx, obj); err != nil {
				self.Recorder.Eventf(obj, nil, v1.EventTypeWarning, "CreateError", "Create", "Failed to Create %s: %s", objName, err.Error())
				return self.handleErr(CreateOp, obj, err)
			}
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Created", "Create", "Created %s", objName)
		} else {
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Updating", "Update", "Updating %s", objName)
			if err := self.Update(ctx, obj); err != nil {
				self.Recorder.Eventf(obj, nil, v1.EventTypeWarning, "UpdateError", "Update", "Failed to update %s: %s", objName, err.Error())
				return self.handleErr(UpdateOp, obj, err)
			}
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Updated", "Update", "Updated %s", objName)
		}
	} else if util.HasFinalizer(obj) {
		if self.StatusID != nil && self.StatusID(obj) != "" {
			sid := self.StatusID(obj)
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Deleting", "Delete", "Deleting %s", objName)
			if err := self.Delete(ctx, obj); err != nil {
				if !ngrok.IsNotFound(err) {
					self.Recorder.Eventf(obj, nil, v1.EventTypeWarning, "DeleteError", "Delete", "Failed to delete %s: %s", objName, err.Error())
			...
			self.Recorder.Eventf(obj, nil, v1.EventTypeNormal, "Deleted", "Delete", "Deleted %s", objName)
		}
...
func (self *BaseController[T]) ReconcileStatus(ctx context.Context, obj T, origErr error) error {
	...
	if err := self.Kube.Status().Update(ctx, obj); err != nil {
		self.Recorder.Eventf(obj, nil, v1.EventTypeWarning, "StatusError", "UpdateStatus", "Failed to reconcile status: %s", err.Error())
