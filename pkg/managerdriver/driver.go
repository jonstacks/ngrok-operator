...
func (d *Driver) updateStatuses(ctx context.Context, c client.Client) error {
	...
	g.Go(func() error {
		return d.updateIngressStatuses(gctx, c)
	})

	g.Go(func() error {
		if err := d.updateHTTPRouteStatuses(gctx, c); err != nil {
			return err
		}
		return d.updateGatewayStatuses(gctx, c)
	})
...
func (d *Driver) updateIngressStatuses(ctx context.Context, c client.Client) error {
	...
	for _, ingress := range needsUpdate {
		g.Go(func() error {
			err := c.Status().Update(gctx, ingress)
			if err != nil {
